...
class S3ConfigMenuDialog extends ComfyDialog {
    createS3Config() {
        let self = this;
        const descEle = $el("p", "When config s3 storage, images and videos of your workflow created will be uploaded to s3 automaticly.")
        ...
        const accessKeyIdInput = $el("input.comfyfile-form-input", { type: 'password', id: "accessKeyId" }, [])
        const accessKeyIdInputText = $el("label.comfyfile-form-label", { for: "accessKeyId" }, ["Access Key ID"])
        accessKeyIdInputText.style.color = "var(--fg-color)";
        accessKeyIdInputText.style.cursor = "pointer";

        const accessSecretKeyInput = $el("input.comfyfile-form-input", { type: 'password', id: "accessSecretKey" }, [])
        const accessSecretKeyInputText = $el("label.comfyfile-form-label", { for: "accessSecretKey" }, ["Access Secret Key"])
        ...
        const regionInputText = $el("label.comfyfile-form-label", { for: "region" }, ["Region"])
        regionInputText.style.color = "var(--fg-color)";
        regionInputText.style.cursor = "pointer";

        const endpointInput = $el("input.comfyfile-form-input", { id: "endpoint" }, [])
        const endpointInputText = $el("label.comfyfile-form-label", { for: "endpoint" }, ["Endpoint Url"])
        ...
        const bucketInputText = $el("label.comfyfile-form-label", { for: "bucket" }, ["Bucket"])
        bucketInputText.style.color = "var(--fg-color)";
        bucketInputText.style.cursor = "pointer";

        const addressingStyleSelect = $el("select.comfyfile-form-input", { id: "addressing_style" }, [])
        ...
        const addressingStyleSelectText = $el("label.comfyfile-form-label", { for: "addressing_style" }, ["Addressing Style"])
        addressingStyleSelectText.style.color = "var(--fg-color)";
        addressingStyleSelectText.style.cursor = "pointer";

        const publicAccessUrlInput = $el("input.comfyfile-form-input", { id: "public_access_url" }, [])
        const publicAccessUrlText = $el("label.comfyfile-form-label", { for: "public_access_url" }, ["Public Access Url"])
        ...
        subMitButton.onclick = (event) => {
            ...
            api.fetchApi(`/comfyfile/save-s3-config`, requestOptions).then(response => response.json())
                .then(data => {
        ...
        testConnectionButton.onclick = (event) => {
            ...
            api.fetchApi(`/comfyfile/test-s3`, requestOptions).then(response => response.json())
                .then(data => {
                    const { success, errMsg } = data;
                    if (!success) {
                        alert("Test Connection failed: " + errMsg)
        ...
        form.append(
            ...[
                ...
                $el("div.comfyfile-form-item", {}, [enable_s3_storage, enable_s3_storage_text]),
                $el("div.comfyfile-form-item", {}, [endpointInputText, endpointInput,]),
                $el("div.comfyfile-form-item", {}, [regionInputText, regionInput,]),
                $el("div.comfyfile-form-item", {}, [accessKeyIdInputText, accessKeyIdInput,]),
                $el("div.comfyfile-form-item", {}, [accessSecretKeyInputText, accessSecretKeyInput,]),
                $el("div.comfyfile-form-item", {}, [bucketInputText, bucketInput,]),
                $el("div.comfyfile-form-item", {}, [addressingStyleSelectText, addressingStyleSelect,]),
                $el("div.comfyfile-form-item", {}, [publicAccessUrlText, publicAccessUrlInput,]),
                subMitButton,
                testConnectionButton
            ]
        )

        api.fetchApi("/comfyfile/get-s3-config").then(response => response.json()).then(response => {
    ...
    constructor() {
        ...
        const content =
            $el("div.comfy-modal-content",
                [
                    ...
                    $el("br", {}, []),
                    $el("div.cm-comfyfile-menu-container",
                        [
                            ...this.createS3Config()
                        ]),
        ...
        this.element = $el("div.comfy-modal", { id: 'cm-comfyfile-dialog', parent: document.body }, [content]);
    }
...
async function install_comfyfile_app(url) {
    if (ComfyfileExplorerMenuDislog.instance) {
        ComfyfileExplorerMenuDislog.instance.startInstall(target);
        try {
            ...
            await ComfyfileExplorerMenuDislog.instance.invalidateControl();
            ComfyfileExplorerMenuDislog.instance.updateMessage("<BR>To apply the installed apps, please click the 'Refresh' button on the main menu.");
        }
    }
}
...
export class ComfyfileExplorerMenuDislog extends ComfyDialog {
    ...
    startInstall(target) {
        const self = ComfyfileExplorerMenuDislog.instance;
    ...
    async createGrid(models_json) {
        ...
        var header_load = document.createElement('th');
        header_load.innerHTML = 'Load';
        header_load.style.width = "50px";
        ...
        headerRow.appendChild(header_load);
        ...
        if (this.data)
            for (var i = 0; i < this.data.length; i++) {
                ...
                var data_load = document.createElement('td');
                var loadBtn = document.createElement('button');
                data_load.style.textAlign = "center";

                loadBtn.innerHTML = 'Load';
                this.install_buttons.push(loadBtn);
    ...
    createHeaderControls() {
        let self = this;
        this.comfyfile_input_box = $el('input.cm-search-filter', { type: 'text', id: 'comfyfile-input-box', placeholder: 'Input comfyfile repo url', value: this.comfyfile_repo }, []);
        ...
        let cell = $el('td', { width: '100%' }, [this.comfyfile_input_box, '  ', install_button]);
...
app.registerExtension({
    ...
    async setupS3Config(menu) {
        ...
        container.onclick = () => {
            if (!comfyfile_s3_instance)
                setS3ConfigInstance(new S3ConfigMenuDialog());
    ...
    async setupComfyfileExplorer(menu) {
        const container = document.createElement("div");
        container.style.background = "linear-gradient(90deg, #00C9FF 0%, #92FE9D 100%)";
        container.style.display = "flex"; // 设置 div 为 Flex 布局
        container.style.width = "100%"
        container.style.borderRadius = "10"
        ...
        button.style.border = 'none'
        button.style.outline = 'none'
        button.style.background = "linear-gradient(90deg, #00C9FF 0%, #92FE9D 100%)";
        button.style.flexGrow = 10
